...
function form_validate(event) {
    // validate form
    // preventing event from submitting form to server side
    // because if there are errors aka incomplete, need to fill

    var errors = false; //assume all is ok
    console.log("Default Errors: " + errors);

    $('.feedback').html('').hide();

// These are validations 1-3
...
// if less than 1 no characters are listed in field
// therefore if no characters, then there are errors = true
...
// Event Listener - we added a class of form="contact"
